                ...
                ForEach(blockedUsers) { user in
                    HStack {
                        ...
                        Button("Unblock") {
                            Task {
                                do {
                                    try await NetworkManager.shared.unblockUser(userId: user.id, authManager: authManager)
                                    // Remove the user from the local list instantly for a smooth UI experience
